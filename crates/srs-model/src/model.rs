...
//! srs-volumetrics' validated value types ([`srs_volumetrics::OilFvf`] /
//! [`srs_volumetrics::GasFvf`]). P-curves aggregate per-realization results via
//! srs-uncertainty (`PercentileSummary`).

use crate::provenance::{PopulationMode, Provenance};
use crate::zones::ZoneTable;
use petekstatic_error::StaticError;
use srs_grid::{build_box, BoxSpec, Dims, Grid, Property};
use srs_gridder::{Conformity, SolveOpts};
use srs_spill::SpillBacking;
use srs_volumetrics::{compute_clipped, Clip, GridSource, InPlace, ZoneVolumes};
use srs_wireframe::{Contact, ContactKind, Wireframe};
use std::path::Path;
use std::sync::Arc;
...
impl Georef {
    ...
    /// [`crate::TrendSurface::with_georef`].
    #[must_use]
...
impl StaticModel {
    ...
    /// ([`crate::StaticModelBuilder`] above budget, R2). The geometry + cubes already
    ...
    /// [`crate::StaticModelTemplate::realize_into`] — every field is overwritten on
    /// the first realize (the grid geometry + cube buffers then grow in place and are
    /// recycled thereafter). The allocating [`crate::StaticModelTemplate::realize`]
    ...
    /// buffer-recycling realize path ([`crate::StaticModelTemplate::realize_into`]).
    pub(crate) fn grid_mut(&mut self) -> &mut Grid {
        &mut self.grid
    }

    /// Overwrite the reused model's non-geometry state after its grid was recycled in
    /// place (crate-internal; [`crate::StaticModelTemplate::realize_into`]). Sets
    ...
    /// (`srs_volumetrics::compute_clipped`), picking the backing: in-core reads the
