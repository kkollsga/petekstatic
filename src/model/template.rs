...
/// [`crate::model::TieMethod::Replace`]), leaving the correlated field to relax away over
...
struct StackState {
    ...
    /// from the source [`crate::model::StackZone::color`] onto each realization's zone table.
    ...
    /// metre scale for [`crate::model::TieSettings`] radius locality (ties are
...
/// of [`crate::model::StaticModelBuilder::with_zone_property`]): the pipeline is run
...
impl StaticModelTemplate {
    ...
    /// contacts** (each [`crate::model::ZoneDraw`] owns an optional GOC/OWC; a contactless
    ...
    ///
    /// # Errors
    /// [`StaticError`] as [`crate::model::StaticModelBuilder::from_horizon_stack`] (bad stack
    ...
    /// [`crate::model::StaticModelBuilder::from_scatter_stack`], so `realize` reproduces
    /// it. The `frame` georef is registered for the realized world frames.
    ///
    /// # Errors
    /// [`StaticError`] as [`Self::from_horizon_stack`] /
    /// [`crate::model::StaticModelBuilder::from_scatter_stack`].
    ...
    /// analog of [`crate::model::StaticModelBuilder::with_collapse_below_m`]: after
    ...
    /// [`crate::model::StaticModelBuilder::with_areal_trend`]. Lateral shape only; the
    /// per-draw prior gives the level. See [`TrendSurface`].
    ///
    /// **Superseded (deprecation-tracked)** — the same interim status as the builder
    /// method it mirrors: the fuller path is [`crate::model::PropertyPipeline`] +
    /// [`crate::model::Gaussian::with_trend`] (collocated cokriging). Retained for the
    ...
    /// [`crate::model::StaticModelBuilder::with_zone_property`]) in the default
    ...
    /// frames — the MC analog of [`crate::model::StaticModelBuilder::with_georef`]. Column
    ...
    /// outline — the MC analog of [`crate::model::StaticModelBuilder::with_boundary`] (finding
    ...
    /// Set how well ties are applied ([`crate::model::TieSettings`]: datum substitution —
    ...
    /// [`crate::model::StaticModelBuilder::with_well_ties`] (`task_petekstatic_template_ties`).
    ...
    pub fn with_well_ties(mut self, ties: Vec<WellTie>) -> Result<Self, StaticError> {
        ...
        // [`crate::model::TieSettings`] — byte-for-byte the builder's tie math
    ...
    /// [`crate::model::StaticModelBuilder::with_extrapolation`]) and — on a stack-aware
    ...
    /// [`crate::model::StaticModelBuilder::with_sugar_cube`]; flows into each realization's
