...
pub struct SectionZone {
    /// The zone's name (matches [`crate::Zone::name`]).
    pub name: String,
...
impl StaticModel {
    ...
    pub fn intersection_bundle(
        ...
        let name_for = |role: srs_wireframe::HorizonRole, fallback: &str| -> String {
            self.framework()
                .horizons
                .iter()
                .find(|h| h.role == role)
        ...
        Ok(IntersectionBundle {
            ...
            top_name: name_for(srs_wireframe::HorizonRole::Top, "TOP"),
            base_name: name_for(srs_wireframe::HorizonRole::Base, "BASE"),
            columns,
            horizon_traces,
            zones: section_zones,
            contacts,
...
fn fence_edge_depths(cell: &Cell, px: f64, py: f64, dx: f64, dy: f64) -> (f64, f64, f64, f64) {
    ...
    let interp = |xx: f64, yy: f64, face: &[srs_grid::Point3]| -> f64 {
...
mod tests {
    ...
    fn utm_world_fence_yields_ordered_columns() {
        ...
        let m = utm_wedge();
        let f = m.map_bundle(&crate::MapSpec::new()).unwrap().frame;
    ...
    fn two_zone_stack_model() -> StaticModel {
        use crate::{HorizonSource, HorizonStack, StackHorizon, StackZone};
        const N: usize = 11;
