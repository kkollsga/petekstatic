...
def test_static_property_workflow_synthetic_perf_budget():
    ...
    project_import = _bench_ms(
        lambda: petekio.Project.import_data(SYNTH_FIXTURE, aliases=ALIASES),
        n=10,
    )
    project = petekio.Project.import_data(SYNTH_FIXTURE, aliases=ALIASES)
    ...
    _assert_under("synthetic Project.import_data min", project_import["min"], 25.0)
...
def test_static_property_workflow_confidential_perf_budget():
    ...
    project_import = _bench_ms(
        lambda: petekio.Project.import_data(CONFIDENTIAL_DATA, aliases=ALIASES),
        n=3,
    )
    project = petekio.Project.import_data(CONFIDENTIAL_DATA, aliases=ALIASES)
    recipe = _build_recipe(project, module=pst)
    ...
    _assert_under("confidential Project.import_data min", project_import["min"], 750.0)
