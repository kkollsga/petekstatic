...
//! (`srs_volumetrics::grv`) **streaming**, one k-slab at a time: peak working set
...
use srs_grid::{hexahedron_volume, Dims, Grid, Pillar, Point3};
use srs_volumetrics::{compute_clipped, CellSlab, Clip, SlabSource, NTG, PORO, SW};
...
impl SpillBacking {
    ...
    /// `srs_volumetrics::GridSource`. The whole spilled volumetric surface
    /// (`bulk_volume`, the model's `in_place*` / `in_place_by_zone`) runs through
    /// the ONE unified core (`srs_volumetrics::compute_clipped`) over this source,
    ...
    pub fn to_in_core_grid(&self) -> Result<Grid, StaticError> {
        let dims = self.dims;
        let mut grid = srs_grid::build_box(srs_grid::BoxSpec::square(
            1.0,
            1.0,
        ...
        for name in self.cube_names().to_vec() {
            ...
            grid.properties_mut()
                .set(srs_grid::Property { name, values })?;
        }
        Ok(grid)
    }
}
