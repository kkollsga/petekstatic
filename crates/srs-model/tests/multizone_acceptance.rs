...
use srs_volumetrics::{NTG, PORO, SW};
use srs_wireframe::{Contact, ContactKind, GriddedDepth, Hardness};
...
fn collapse_conserves_volume_marks_cells_and_spares_thick_zone() {
    ...
    assert!(yes
        ...
        .warnings
        .iter()
        .any(|w| matches!(w, srs_model::BuildWarning::CellsCollapsed { cells } if *cells > 0)));
...
fn per_zone_priors_set_distinct_zone_levels_and_conserve_volume() {
    ...
    let ntg = m.zone_stats(NTG).unwrap();
    let by =
        |v: &[srs_model::ZoneStat], name: &str| v.iter().find(|s| s.zone == name).unwrap().clone();
    ...
    let m = StaticModelBuilder::from_horizon_stack(wedge_stack(), opts())
        ...
        .with_zone_property(
            ...
            PropertyPipeline::new(PORO)
                ...
                .propagate(srs_model::Gaussian::new(vgm, 7).allow_mean_fill()),
        )
        .build()
        .unwrap();
