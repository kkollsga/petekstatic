...
use srs_grid::{CornerPointGeom, Dims, Grid, Pillar, Point3};
...
/// ([`srs_grid::Grid::install_geometry`]).
///
/// # Errors
...
mod tests {
    use super::*;
    use crate::surface::{solve_surface, Control, SolveOpts};
    ...
    fn flat_surfaces_proportional_recover_a_box() {
        ...
        assert!((grid.cell(srs_grid::Ijk::new(0, 0, 0)).dz() - 10.0).abs() < 1e-6);
    }
    ...
    fn proportional_layers_split_thickness_evenly() {
        ...
        for k in 0..4 {
            let dz = lg.grid.cell(srs_grid::Ijk::new(0, 0, k)).dz();
    ...
    fn follow_top_derives_nk_and_layers_parallel_to_the_top() {
        ...
        for k in 0..1 {
            for i in [0usize, 5, 9] {
                let dz = lg.grid.cell(srs_grid::Ijk::new(i, 0, k)).dz();
    ...
    fn follow_base_mirrors_follow_top() {
        ...
        let dz = lg.grid.cell(srs_grid::Ijk::new(9, 0, lg.nk - 1)).dz();
    ...
    fn tilted_top_follows_structure() {
        ...
        let grid = layer_grid(&top, &base, 100.0, 100.0, 1, Conformity::Proportional)
            .unwrap()
            .grid;
        let updip = grid.cell(srs_grid::Ijk::new(0, 0, 0)).dz();
        let downdip = grid.cell(srs_grid::Ijk::new(9, 0, 0)).dz();
        assert!(
            updip > downdip,
    ...
    fn follow_top_pinches_out_flat_column() {
        ...
        .unwrap();
        assert_eq!(lg.nk, 2);
        let l0 = lg.grid.cell(srs_grid::Ijk::new(0, 0, 0)).dz();
        let l1 = lg.grid.cell(srs_grid::Ijk::new(0, 0, 1)).dz();
    ...
    fn stack_concatenates_zones_and_partitions_k() {
        ...
        assert!((sg.grid.cell(srs_grid::Ijk::new(0, 0, 0)).dz() - 12.5).abs() < 1e-6);
        assert!((sg.grid.cell(srs_grid::Ijk::new(0, 0, 4)).dz() - 40.0 / 3.0).abs() < 1e-6);
    ...
    fn collapse_conserves_volume_and_stays_within_zones() {
        ...
        for k in yes.zones[1].k_range() {
            for j in 0..3 {
                for i in 0..3 {
                    let dz = yes.grid.cell(srs_grid::Ijk::new(i, j, k)).dz();
