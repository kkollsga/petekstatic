...
//!
//! This supersedes the interim [`crate::TrendSurface`] multiplier hook (which only
...
use rayon::prelude::*;
use srs_grid::{Grid, Ijk, Property};
use srs_petro::{arithmetic_mean, geometric_mean, harmonic_mean, WeightedSample};
...
impl PropertyPipeline {
    ...
    /// ([`crate::StaticModelBuilder::with_zone_property`]).
    ///
...
/// 4-corner means ([`srs_grid::Cell::top_depth`]/`bottom_depth`, the column
...
fn cell_depth_at_xy(cell: &srs_grid::Cell, wx: f64, wy: f64) -> (f64, f64) {
    let c = &cell.corners;
...
mod tests {
    ...
    use srs_grid::{build_box, BoxSpec, Dims};
