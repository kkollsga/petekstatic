...
use srs_petro::{upscale_porosity, upscale_sw, SwSample, WeightedSample};
use srs_volumetrics::{populate_constant, validate_fraction, ConstantPriors, NTG, PORO, SW};
...
fn apply_areal_trend(grid: &mut Grid, trend: &TrendSurface) -> Result<(), StaticError> {
    ...
    let lattice = crate::pipeline::areal_lattice(grid)?;
